...
const ASCII_FRAMES = ["[|....]", "[/....]", "[-....]", "[\\....]"];
const INSTAGRAM_MEDIA_PAGE_SIZE = 100;
...
export default function InstagramManager() {
  ...
  const loadAllMedia = async ({ targetIgUserId, seedPayload = null }) => {
    const merged = [];
    ...
    if (seedPayload) {
      ...
      const payload = await fetchMediaPage({ targetIgUserId });
    ...
    while (
      nextCursor
      && (targetMediaCount === null || merged.length < targetMediaCount)
    ) {
      ...
      const payload = await fetchMediaPage({ targetIgUserId, afterCursor: nextCursor });
      nextCursor = consumePage(payload);
      pagesFetched += 1;
    }
  ...
  const refreshMedia = async () => {
    ...
    setMediaError("");
    try {
      const payload = await loadAllMedia({ targetIgUserId: igUserId });
  ...
  const handleBoot = async (event) => {
    ...
    try {
      ...
      const loaded = await loadAllMedia({
        targetIgUserId: payload.ig_user_id,
        seedPayload: payload,
      });
      setContext({
        ...payload,
        profile: loaded.profile || payload.profile,
      });
